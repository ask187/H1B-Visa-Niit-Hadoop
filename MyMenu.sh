...
show_menu
    ...
    if [[ $opt = "" ]]; then 
		...
		hadoop jar FinalFirst.jar firsta /niit/mainFolder /niit/F1P2
		hadoop fs -cat /niit/F1P2/p*
	    ...
	    hive -e "select worksite,count(*) as total from h1b_final where job_title like '%DATA ENGINEER%' and year=$var group by worksite order by total desc limit 5;"
        show_menu;
        ;;
	...
	4) clear;
	    ...
	    hive -e "select worksite,count(*) as total from h1b_final where case_status='CERTIFIED' and year=$var group by worksite order by total desc limit 5;"
       show_menu;
        ;;  
	...
	5) clear;
	    ...
	    hadoop jar FinalThird.jar MyDC /niit/mainFolder /niit/finalPresentation1
	    hadoop fs -cat /niit/finalPresentation1/p*
       show_menu;
        ;;
	    ...
	    hive -e "select employer_name,count(*) as total from h1b_final where year=$var group by employer_name order by total desc limit 5;"
         show_menu;
        ;;
	    ...
	    hive -e "select job_title,count(*) as total from h1b_final where year=$var group by job_title order by total desc limit 10;"
        show_menu;
        ;;
	    ...
	    hive -e "select job_title,count(*) as total from h1b_final where case_status='CERTIFIED' and year=$var group by job_title order by total desc limit 10;"
        show_menu;
        ;;
	...
	11) clear;
	    ...
	    case $n in
                1)	echo "Full time"
                    pig -x local final8a.pig
                    ;;		
                    
                2) 	echo "Part time"
                   pig -x local final8b.pig
                    ;;
	...
	option_picked "10) Which are the top 10 job positions which have the  success rate more than 70% in petitions and total petitions filed more than 1000?"
		pig -x local final10.pig
        show_menu;
        ;;

	14) clear;
	option_picked "11) Export result for question no 10 to MySql database."
		mysql -u root -p'root' -e 'drop database question11;create database if not exists question11;use question11;create table question11(job_title varchar(100),petitions int,success_rate float);';

		sqoop export --connect jdbc:mysql://localhost/question11 --username root --password 'helloworld' --table question11 --update-mode allowinsert  --export-dir /Home/hduser/niit/Pig1/question10/part-r-00000 --input-fields-terminated-by '\t';
